...
- Status: accepted; **History on-demand Explain implemented** (default LLM off; GPU-local
  optional). Prose **cache + invalidation** still open (contract §14.6).
- Date: 2026-07-12
...
versions, and load facts. History currently surfaces those codes (for example
`development.adjustment.increase · rule 0.0.1-development`) without plain-language
explanation. Phase 3 and the product vision call for clearer, inspectable explanations
and explanation-comprehension as a beta measure.
...
explicit. **Which model weights, quantisation, or inference engine an operator runs is
not a product decision in this ADR.** The application binds to a versioned generation
port and groundedness contract; model choice remains configuration and evaluation
evidence.
...
2. **Authority.** Deterministic TypeScript methodology and application rules remain the
   ...
   truth.
3. **Model-agnostic port.** The product depends on an `ExplanationGenerationPort` and on
...
5. **Delivery.** Generation is lazy or post-commit. It must not block set completion or
   other gym critical paths. Prose may be cached keyed by decision identity, model id,
   model content digest, and prompt version.
...
7. **Implementation gate.** No product routes, SDKs, tables, navigation, or flags ship
   until the [explanation generation contract](../EXPLANATION_GENERATION_CONTRACT.md)
   acceptance criteria for the chosen slice are met. This ADR pins design; it does not
   authorize scaffold-only code.
...
- Pinning a specific model family, size, quantisation, weight format, or inference engine
  in product architecture

## Consequences

- Self-hosting contract may later list optional local inference under a configured model
  directory and loopback endpoint; it remains non-authoritative and non-mandatory.
...
- Choosing or changing the operator’s model is a configuration and evaluation matter:
  groundedness tests and digests pin *what ran*, not a forever product default.
