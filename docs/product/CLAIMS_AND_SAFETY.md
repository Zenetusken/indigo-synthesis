...
| Optional local plain-language paraphrase of a persisted reason code | Inferred presentation only (History shipped; instance default is off) | Never a decision source; label as inferred; always show reason code and ruleset version; see [explanation generation contract](../architecture/EXPLANATION_GENERATION_CONTRACT.md) |
