...
| J5 — Complete and learn | Transactional completion, immutable completed sets/history/decisions, terminal feedback guards, a narrow audited monotonic post-completion pain-report correction, and a new future program revision without rewriting the completed revision | Main browser journey, direct database integrity tests, adjustment property/unit tests, and completion-replay integration | The adjustment policy is a development test policy, not coaching advice; general audited set correction and richer Progress views are Phase 3 |
...
| Database integrity | Four committed Drizzle migrations, PostgreSQL 18 preflight, ownership/lifecycle checks, unique constraints, terminal-history and published-prescription guards, audit immutability, and terminal/monotonic feedback enforcement | Keep fresh-migration/preflight proof in the final release evidence |
...
| Safety honesty | Contraindication/restriction block, fail-closed content status, pain stop/hold, advanced-tier denial, no diagnosis, and no fabricated substitution | Human strength and safety approval of the intended population, movements, bounds, stop rules, and copy |
...
At this snapshot, Biome, TypeScript, unit/domain plus six architecture guards,
database integration, the seven-path Playwright suite, PostgreSQL preflight/fresh
migration, and the reviewed-mode production build are green. The Playwright suite runs
against a freshly recreated PostgreSQL database with application APIs unmocked.
