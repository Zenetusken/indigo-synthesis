...
| Optional local grounded explanation **prose cache + Program page** | History on-demand Explain is implemented (default LLM off; `pnpm test:e2e:llm` for GPU path). Cache/invalidation and Program surface not built | Multi-run product-path archive green (`pnpm llm:archive-product-path`); PG cache keyed by `explanationCacheKey` with subject-scoped deletion; Program Explain only after cache measured |
