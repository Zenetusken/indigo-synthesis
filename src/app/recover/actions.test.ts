...
import { recoverOwnerAction } from './actions'
...
const actionMocks = vi.hoisted(() => ({
  ...
  command: Object.freeze({ kind: 'nominal-owner-recovery-command' }),
}))
...
vi.mock('@/modules/identity/server/recovery-redemption-command', () => ({
  captureOwnerRecoveryRedemptionMutationCommand: actionMocks.capture,
}))
...
describe('owner-recovery redemption action', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    ...
    actionMocks.getPort.mockReturnValue({ redeemOwnerRecovery: actionMocks.redeem })
  })
  ...
  it('captures the nominal command at action entry and redirects only after redemption', async () => {
    ...
    await expect(recoverOwnerAction(initialState, formData)).resolves.toBe(undefined)
    ...
    expect(actionMocks.redirect).toHaveBeenCalledWith('/sign-in?recovered=1')
  ...
  it('keeps ingress denial outside the public credential-failure set', async () => {
    ...
    await expect(recoverOwnerAction(initialState, recoveryForm())).resolves.toEqual({
      kind: 'rejected',
  ...
  it('invalidates an expired or stale form while keeping its public response uniform', async () => {
    ...
    expect(actionMocks.revalidatePath).toHaveBeenCalledWith('/recover')
    expect(actionMocks.redirect).not.toHaveBeenCalled()
  })
})
