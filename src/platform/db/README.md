...
- one ordered committed SQL migration ledger with 16 current entries;
...
- startup preflight for PostgreSQL 18 or newer, every current committed migration hash,
  canonical 0004 ledger provenance, required columns/indexes, the explanation-cache
  contract, all 28 exact enabled trigger/table/function bindings, and reviewed-mode
  content eligibility. Extra historical ledger rows from a development branch do not
  false-fail when every current hash and concrete invariant still holds.
