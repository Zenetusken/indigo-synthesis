...
/**
 * Serializes the JSON value with recursively sorted object keys. Arrays retain their
 * domain-significant order. Undefined values and non-finite numbers are rejected rather
 * than being silently discarded or rewritten.
 */
export function canonicalStringify(value: CanonicalValue): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) {
      throw new NonCanonicalValueError('Canonical numbers must be finite.')
    }
...
/** A small, synchronous SHA-256 implementation with no clock, I/O, or platform import. */
export function sha256(value: string): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h + sum1 + choice + (sha256Constants[index] ?? 0) + (words[index] ?? 0)) >>> 0
